...
-   [:material-message-processing: **LLM Decode**](llm_decode.md)
    ...
    Exercises dependent KV-cache gathers and small projection-like math to isolate autoregressive token-generation pressure.

-   [:material-text-long: **LLM Prefill**](llm_prefill.md)

    ---

    Exercises causal context scans and projection-like math to isolate long-prompt processing pressure.

-   [:material-database-sync: **KV Cache Churn**](kv_cache_churn.md)

    ---

    Exercises sparse page reads and updates that resemble paged, ragged KV-cache maintenance.
