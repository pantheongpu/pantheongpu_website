...
The explorer carries **Mem Type** and **Mem Vendor** columns, and every
published record reads N/A in both. That is accurate rather than missing: no
released version of Pantheon collects them, so nothing has ever been
measured to put there. The columns are reserved for a planned release that
would read memory type (GDDR6, GDDR7, HBM3 and so on) and vendor (Samsung,
SK hynix, Micron) from the driver's own memory information table on NVIDIA
and from `rocm-smi` on AMD.

When it ships, read those values as the vendor the board's VBIOS was
configured for -- one value per card, not a survey of the chips. A repaired
card fitted with another vendor's memory would still report the original, so
the declared vendor is what the column means; the memory diagnostics are
what measure the chips themselves.
...
| CUDA | 12.6 | 560.94 | 1 |
| CUDA | 12.8 | 570.148.08, 570.158.01, 570.195.03, 575.57.08, 580.65.06, 580.95.05, 580.105.08, 580.126.09, 580.126.20, 580.159.03, 580.159.04, 580.173.02, 595.71.05, 595.84, 610.57.04 | 23 |
| CUDA | 12.9 | 595.91.07 | 1 |
| CUDA | 13.0 | 595.71, 595.91.07, 595.97 | 7 |
| CUDA | 13.2 | 595.91.07, 596.36 | 9 |
